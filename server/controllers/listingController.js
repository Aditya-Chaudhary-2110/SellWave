...
export const getAllPublicListing = async (req, res) => {
  try {
    ...
    const safeListings = listings.map((l) => ({
      ...
      followers_count: Number(l.followers_count) || 0,
      engagement_rate: Number(l.engagement_rate) || 0,
      monthly_views: Number(l.monthly_views) || 0,
    }));

    return res.json({ listings: safeListings });
  } catch (error) {
    console.log(error);
...
export const getAllUserListing = async (req, res) => {
  try {
    ...
    return res.json({ listings: safeListings, balance });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: error.message });
  }
...
export const updateListing = async (req, res) => {
  try {
    ...
    accountDetails.price = Number(accountDetails.price) || 0;
    accountDetails.followers_count =
      Number(accountDetails.followers_count) || 0;
    accountDetails.engagement_rate =
      Number(accountDetails.engagement_rate) || 0;
    accountDetails.monthly_views = Number(accountDetails.monthly_views) || 0;
    accountDetails.platform = accountDetails.platform.toLowerCase();
    accountDetails.niche = accountDetails.niche.toLowerCase();
    ...
    let images = accountDetails.images || [];
    ...
    if (req.files?.length) {
      ...
      images = [...images, ...uploaded.map((i) => i.url)];
    }

    const listing = await prisma.listing.update({
      where: { id: accountDetails.id },
      data: { ...accountDetails, images },
    });
...
export const toggleStatus = async (req, res) => {
  try {
    ...
    if (listing.status === "active" || listing.status === "inactive") {
      ...
      return res.status(404).json({ message: "Your listing is banned" });
    } else if (listing.status === "sold") {
      return res.status(404).json({ message: "Your listing is sold" });
    }
  ...
  try {
    ...
    if (listing.ownerId === userId) {
      return res.status(400).json({
        message: "You con't purchase your own listing",
      });
    }
    ...
    const stripeInstance = new Stripe(process.env.STRIPE_SECRET_KEY); // FIXED
