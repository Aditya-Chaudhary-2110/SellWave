...
const sendPurchaseEmail = inngest.createFunction(
  ...
  async ({ event }) => {
    ...
    await sendEmail({
     ...
     <div> ${credential.updatedCredential.map((cred) => `<p> ${cred.name} : ${cred.value}  </p>`).join("")}</div>
     <p>If you have any questions, please contact us at <a href="mailto:support@example.com">support@example.com</p>
      `,
    });
  },
);
